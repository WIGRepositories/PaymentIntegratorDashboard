...
app.controller('myCtrl', function ($scope, $http, $localStorage, $uibModal) {
    ...
    $scope.UserRegistrationCol = 'College Name,College Registration Number,College Address,Mobile Number,EmailId'
    $scope.UserRegistrationArr = [{ "Id": 1, "College Name": "College Registration Number" },
                        { "Id": 2, "College Name": "College Registration Number" }]
    ...
    $scope.SetOptionSettings = function () {
        switch ($scope.seloption) {
            ...
            case "13":
                $scope.mandatoryCols = $scope.UserRegistrationCol;
                //  $scope.optionsCols = 'Address,phone,emailid';
                ...
                $scope.processData = function (allText) {
                    ...
                    for (var i = 1; i < allTextLines.length; i++) {
                        ...
                        var data = allTextLines[i].split(',');
                        if (data == '') continue;
                        lines.push(GetUserRegistration(data));
                    ...
                    var req = {
                        method: 'POST',
                        url: '/api/DataLoad/RegistrationDetails',
                        data: lines
                    }
                    $http(req).then(function (res) {
                ...
                function GetUserRegistration(data) {

                    var list = {
                        CollegeName: data[0],
                        CollegeRegistrationNumber: data[1],
                        CollegeAddress: data[2],
                        MobileNumber: data[3],
                        EmailId: data[4],                       
                       flag: 'I'
                    }
